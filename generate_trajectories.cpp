...
void generate_trajectories( double inital_number_of_cases, double param_beta, double t0, double tf, double h )
{
    ...
    // death prob home
    bool using_default_death_home = (ppc->tvp_death_home->get_day_index() == 0) && (ppc->tvp_death_home->get_current_begin_day() > 0);
    double next_end_day_tvp_death_home = using_default_death_home ? ((double)ppc->tvp_death_home->get_current_begin_day()) - 0.000001 : ((double)ppc->tvp_death_home->get_current_end_day()) + 0.999999; 
    bool end_tvp_death_home = false;
    ...
    // vaccine efficacy
    bool using_default_vac_efficacy = (ppc->tvp_vac_efficacy->get_day_index() == 0) && (ppc->tvp_vac_efficacy->get_current_begin_day() > 0);
    double next_end_day_tvp_vac_efficacy = using_default_vac_efficacy ? ((double)ppc->tvp_vac_efficacy->get_current_begin_day()) - 0.000001 : ((double)ppc->tvp_vac_efficacy->get_current_end_day()) + 0.999999; 
    bool end_tvp_vac_efficacy = false;

    // treatment coverage
    bool using_default_treat_cov = (ppc->tvp_treatment_coverage->get_day_index() == 0) && (ppc->tvp_treatment_coverage->get_current_begin_day() > 0);
    double next_end_day_tvp_treat_cov = using_default_treat_cov ? ((double)ppc->tvp_treatment_coverage->get_current_begin_day()) - 0.000001 : ((double)ppc->tvp_treatment_coverage->get_current_end_day()) + 0.999999; 
    bool end_tvp_treat_cov = false;
    ...
    // if the first tv-incub-period-beginday CLO is higher than 0, use the default incubation period of 6 first 
    bool using_default_incub_period = (ppc->tvp_incubation_period->get_day_index() == 0) && (ppc->tvp_incubation_period->get_current_begin_day() > 0);
    // update at the end of end_day
    double next_end_day_tvp_incubation_time = using_default_incub_period ? ((double)ppc->tvp_incubation_period->get_current_begin_day()) - 0.000001 : ((double)ppc->tvp_incubation_period->get_current_end_day()) + 0.999999; 
    bool end_tvp_incubation_time = false;

    // if the first tv-infect-period-beginday CLO is higher than 0, use the default infectious period of 7 + G_CLO_TIME_SYMPTOHOSP
    bool using_default_infect_period = (ppc->tvp_infectious_period->get_day_index() == 0) && (ppc->tvp_infectious_period->get_current_begin_day() > 0);
    // update at the end of end_day
    double next_end_day_tvp_infect_period = using_default_infect_period ? ((double)ppc->tvp_infectious_period->get_current_begin_day()) - 0.000001 : ((double)ppc->tvp_infectious_period->get_current_end_day()) + 0.999999; 
    bool end_tvp_infect_period = false;
    ...
    {                                       // but we do anyway because tt is just barely below ttstop by some tiny fraction
        ...
        // time-varying death prob home
        if( tt > next_end_day_tvp_death_home && tt < next_end_day_tvp_death_home + rkstep && !end_tvp_death_home )
        {
            // fprintf(stdout, "\n%f v_prob_I4_D[8] %f\n", tt, ppc->v_prob_I4_D[8]);
            if (!using_default_death_home){
                end_tvp_death_home = !ppc->tvp_death_home->advance_indices(); 
                for (int ac = 0; ac < NUMAC; ac++){
                    ppc->v_prob_I4_D[ac] = ppc->tvp_death_home->get_current_data(ac);
                    // fprintf(stdout, "v_prob_I4_D[%d] %f\n", ac, ppc->v_prob_I4_D[ac]);
                }
            } else {
                ...
                ppc->v_prob_I4_D[7] = 0.05;
                ppc->v_prob_I4_D[8] = 0.22;
                using_default_death_home = false; // reset so that advance_indices() can be called next time
            }
            // fprintf(stdout, "\nv_prob_I4_D[8] %f\n", ppc->v_prob_I4_D[8]);
                        
            // printf("\n#### death prob home state ####\ntime %.3f\n", tt);
            // ppc->tvp_death_home->print_current_state();
            next_end_day_tvp_death_home = (double)ppc->tvp_death_home->get_current_end_day() + 0.999999; // update at the end of end_day

        }

        // vaccination
        // vaccine efficacy
        if( tt > next_end_day_tvp_vac_efficacy && tt < next_end_day_tvp_vac_efficacy + rkstep && !end_tvp_vac_efficacy )
        {
            // printf("\n#### tvp_vac_efficacy state ####\ntime %.3f\tvalue %.3f\n", tt, ppc->tvp_vac_efficacy->get_current_data());
            if (using_default_vac_efficacy){
                using_default_vac_efficacy = false; // reset so that advance_indices() can be called next time
                next_end_day_tvp_vac_efficacy = (double)ppc->tvp_vac_efficacy->get_current_begin_day() - 0.000001;
            } 
            else {
                end_tvp_vac_efficacy = !ppc->tvp_vac_efficacy->advance_indices();                 
                next_end_day_tvp_vac_efficacy = (double)ppc->tvp_vac_efficacy->get_current_end_day() + 0.999999; // update at the end of end_day
            }
            // printf("\nafter update tvp_vac_efficacy state ##\ntime %.3f\t value %.3f\n", tt, ppc->tvp_vac_efficacy->get_current_data());
        }
        // actual vaccinating step
        ...
        if (tt > next_start_day_tvp_vac_num && tt < next_end_day_tvp_vac_num  + 0.999999 && !end_tvp_vac_num ){
            ...
            for (int ac = 0; ac < NUMAC; ac++){
                ...
                {
                    double sum_SEAR, frac_R, frac_RHOSP = 0.0;
                    double vac_eff = ppc->tvp_vac_efficacy->get_current_data();

                    sum_SEAR = yic[ac]; // S
                    for (i = 0; i < NUME; i++){ sum_SEAR += yic[STARTE + i*NUMAC + ac]; } // S + E
                    for (i = 0; i < NUMA; i++){ sum_SEAR += yic[STARTA + i*NUMAC + ac]; } // S + E + A
                    sum_SEAR += yic[STARTR + ac]; // S + E + A + R
                    sum_SEAR += yic[STARTRHOSP + ac]; // S + E + A + R + RHOSP
                    // sum_SEAR += yic[STARTVAX + ac]; // S + E + A + R + RHOSP + VAX

                    frac_vac_seroneg = num_vaccinees * yic[ac] / sum_SEAR; // S / (S + E + A + R + RHOSP + VAX)
                    frac_R = num_vaccinees * yic[STARTR + ac] / sum_SEAR;
                    frac_RHOSP = num_vaccinees * yic[STARTRHOSP + ac] / sum_SEAR;

                    frac_vac_seroneg = (frac_vac_seroneg < yic[ac]) ? frac_vac_seroneg : yic[ac];
                    frac_R = (frac_R < yic[STARTR + ac]) ? frac_R : yic[STARTR + ac];
                    frac_RHOSP = (frac_RHOSP < yic[STARTRHOSP + ac]) ? frac_RHOSP : yic[STARTRHOSP + ac];

                    yic[ac] -= frac_vac_seroneg * vac_eff;
                    yic[STARTR + ac] -= frac_R * vac_eff;
                    yic[STARTRHOSP + ac] -= frac_RHOSP * vac_eff;

                    yic[STARTVAX + ac] += (frac_vac_seroneg + frac_R + frac_RHOSP) * vac_eff;
                }
        ...
        // treatment coverage
        if( tt > next_end_day_tvp_treat_cov && tt < next_end_day_tvp_treat_cov + rkstep && !end_tvp_treat_cov )
        {
            // printf("\n#### tvp_treatment_coverage state ####\ntime %.3f\t \n", tt); ppc->tvp_treatment_coverage->print_current_state(); 
            if (using_default_treat_cov){
                using_default_treat_cov = false; // reset so that advance_indices() can be called next time
                next_end_day_tvp_treat_cov = (double)ppc->tvp_treatment_coverage->get_current_begin_day() - 0.000001;
            } 
            else {
                end_tvp_treat_cov = !ppc->tvp_treatment_coverage->advance_indices();                 
                next_end_day_tvp_treat_cov = (double)ppc->tvp_treatment_coverage->get_current_end_day() + 0.999999; // update at the end of end_day
            }
            // printf("\nafter update tvp_vac_efficacy state ##\ntime %.3f\t value\n", tt); ppc->tvp_treatment_coverage->print_current_state();
        }
        ...
        // check if needs to update incubation period
        if( tt > next_end_day_tvp_incubation_time && tt < next_end_day_tvp_incubation_time + rkstep && !end_tvp_incubation_time )
        {
            // printf("\n#### tvp_incubation_period state ####\ntime %.3f\ni_len_incub_period value %.3f\n", tt, ppc->v[ i_len_incub_period ]);

            if (using_default_incub_period){
                using_default_incub_period = false; // reset so that advance_indices() can be called next time

                next_end_day_tvp_incubation_time = (double)ppc->tvp_incubation_period->get_current_begin_day() - 0.000001;
            } 
            else {
                end_tvp_incubation_time = !ppc->tvp_incubation_period->advance_indices(); 

                ppc->v[ i_len_incub_period ] = ppc->tvp_incubation_period->get_current_data();
                
                next_end_day_tvp_incubation_time = (double)ppc->tvp_incubation_period->get_current_end_day() + 0.999999; // update at the end of end_day
            }

            // printf("\nafter update tvp_incubation_period state ##\ntime %.3f\ni_len_incub_period value %.3f\n", tt, ppc->v[ i_len_incub_period ]);
            // ppc->tvp_incubation_period->print_current_state();
        }


        // check if needs to update incubation period
        if( tt > next_end_day_tvp_infect_period && tt < next_end_day_tvp_infect_period + rkstep && !end_tvp_infect_period )
        {
            // printf("\n#### tvp_infectious_period state ####\ntime %.3f\ni_len_symptomatic_infectious_period_phase_2 value %.3f\n", tt, ppc->v[ i_len_symptomatic_infectious_period_phase_2 ]);

            if (using_default_infect_period){
                using_default_infect_period = false; // reset so that advance_indices() can be called next time

                next_end_day_tvp_infect_period = (double)ppc->tvp_infectious_period->get_current_begin_day() - 0.000001;
            } 
            else {
                end_tvp_infect_period = !ppc->tvp_infectious_period->advance_indices(); 

                ppc->v[ i_len_symptomatic_infectious_period_phase_2 ] = ppc->tvp_infectious_period->get_current_data() - ppc->v[ i_len_symptomatic_infectious_period_phase_1 ];
                
                next_end_day_tvp_infect_period = (double)ppc->tvp_infectious_period->get_current_end_day() + 0.999999; // update at the end of end_day
            }

            // printf("\nafter update tvp_infectious_period state ##\ntime %.3f\ni_len_symptomatic_infectious_period_phase_2 value %.3f\n", tt, ppc->v[ i_len_symptomatic_infectious_period_phase_2 ]);
            // ppc->tvp_infectious_period->print_current_state();
        }
        ...
        if( counter%(30*steps_per_day)==0 && G_B_CHECKPOP_MODE )
        {
            totalpop=0.0;
            // for(int k=0; k<dim-18; k++) totalpop += yic[k];
            for(int k=0; k<STARTJ; k++) totalpop += yic[k];
            printf("\n\t t %f totalpop=%1.16f", tt, totalpop);
