...
//     printf("\n\n");
    ...
    ppc->calculate_prob_I2_H_treatment();
    ...
    // if tv-vac-efficacy is not set, use default
    if (ppc->tvp_vac_efficacy->empty_data()){
        ppc->tvp_vac_efficacy->append_data(1.0);
    }
    if ( ppc->tvp_vac_efficacy->fill_missing_days() ){
        ppc->tvp_vac_efficacy->advance_indices(); // start day_index and data_index at 0
    } else {
        fprintf(stderr,"\n\n\tError in parsing time-varying vaccine efficacy.\n\tPlease check all -tv-vac-efficacy-* command-line options or unset all these arguments to use the default values.\n\n");
        ppc->tvp_vac_efficacy->print_all();
        exit(-1);
    }
    // ppc->tvp_vac_efficacy->print_all();
    // exit(0);
    ...
    // treatment coverage
    if (ppc->tvp_treatment_coverage->empty_data()){
        ppc->tvp_treatment_coverage->append_data(0, 0.0);
        ppc->tvp_treatment_coverage->append_data(1, 0.0);
        ppc->tvp_treatment_coverage->append_data(2, 0.0);
        ppc->tvp_treatment_coverage->append_data(3, 0.0);
        ppc->tvp_treatment_coverage->append_data(4, 0.0);
        ppc->tvp_treatment_coverage->append_data(5, 0.0);
        ppc->tvp_treatment_coverage->append_data(6, 0.0);
        ppc->tvp_treatment_coverage->append_data(7, 0.0);
        ppc->tvp_treatment_coverage->append_data(8, 0.0);
    }
    if ( ppc->tvp_treatment_coverage->fill_missing_days() ){
        ppc->tvp_treatment_coverage->advance_indices(); // start day_index and data_index at 0
    } else {
        fprintf(stderr,"\n\n\tError in parsing time-varying treatment coverage.\n\tPlease check all -tv-treat-cov-* command-line options or unset all these arguments to use the default values.\n\n");
        ppc->tvp_treatment_coverage->print_all();
        exit(-1);
    }
    // ppc->tvp_treatment_coverage->print_all();
    ...
    // if tv-incub-time is not set, use default
    if (ppc->tvp_incubation_period->empty_data()){
        ppc->tvp_incubation_period->append_data(7.0);
    }
    if ( ppc->tvp_incubation_period->fill_missing_days() ){
        ppc->tvp_incubation_period->advance_indices(); // start day_index and data_index at 0
    } else {
        fprintf(stderr,"\n\n\tError in parsing time-varying incubation time.\n\tPlease check all -tv-incub-period-* command-line options or unset all these arguments to use the default values.\n\n");
        ppc->tvp_incubation_period->print_all();
        exit(-1);
    }
    // ppc->tvp_incubation_period->print_all();
    ...
    // if tv-infect-period is not set, use default
    if (ppc->tvp_infectious_period->empty_data()){
        ppc->tvp_infectious_period->append_data(14.0); // default 7 symp-hosp + 7 symp w/o hosp
    }
    if ( ppc->tvp_infectious_period->fill_missing_days() ){
        ppc->tvp_infectious_period->advance_indices(); // start day_index and data_index at 0
    } else {
        fprintf(stderr,"\n\n\tError in parsing time-varying infectious time.\n\tPlease check all -tv-infect-period-* command-line options or unset all these arguments to use the default values.\n\n");
        ppc->tvp_infectious_period->print_all();
        exit(-1);
    }
    // make sure the current infectious period is longer than the time from symp to hosp
    if (ppc->tvp_infectious_period->get_current_data() > G_CLO_TIME_SYMPTOHOSP ){ /* do nothing */ } 
    else {
        fprintf(stderr,"\n\n\tInfectious time must be longer than time from symptomatic to hospitalization.\n\tPlease check -tv-infect-period and -time-symp-to-hosp command-line options or unset all these arguments to use the default values.\n\n");
        ppc->tvp_infectious_period->print_all();
        exit(-1);
    }
   ...
   return 1;*/
    ...
    //
    // default run: no treatment
    if (ppc->v[i_treatment_beginday] < 0 ){ ppc->v[i_treatment_beginday] = G_CLO_TF + 2; }
    
    // incubation period
    //
    // ppc->v[ i_len_incub_period ]                            =  6.0;  // this is an average of the Lauer et al estimate (5.5d) and Backer et al (6.5d)
    if (ppc->tvp_incubation_period->get_day_index() == 0 && ppc->tvp_incubation_period->get_current_begin_day() > 0){
        // use the default pre-omicron value
        // this is an average of the Lauer et al estimate (5.5d) and Backer et al (6.5d)
        ppc->v[ i_len_incub_period ] = 6.0;
    } else {
        ppc->v[ i_len_incub_period ] = ppc->tvp_incubation_period->get_current_data();
    }

    // infectious period and time symp to hosp
    //
    ...
    // by default, the total infectious period = G_CLO_TIME_SYMPTOHOSP + phase 2 of the symptomatic period = G_CLO_TIME_SYMPTOHOSP + 7
    if (ppc->tvp_infectious_period->get_day_index() == 0 && ppc->tvp_infectious_period->get_current_begin_day() > 0){
        // use the default pre-omicron value
        ppc->v[ i_len_symptomatic_infectious_period_phase_2 ] = 7.0;
    } else {
        ppc->v[ i_len_symptomatic_infectious_period_phase_2 ] = ppc->tvp_infectious_period->get_current_data() - G_CLO_TIME_SYMPTOHOSP;
    }
...
/*     ppc->v_prob_I2_H[0] = G_CLO_HOSPFRAC_10;    // NOTE because there is not likely to be data on hosp rates for 0-9 y.o., we adopt the hosp rate for 10-19 year olds here
    ...
    // TODO if length of all death home are not identical, append 0 to shorter vectors
    // if no death home is set via command-line options, use default
    if (ppc->tvp_death_home->empty_data() ) {
        ppc->tvp_death_home->append_data(0, 0.0);
        ppc->tvp_death_home->append_data(1, 0.0);
        ppc->tvp_death_home->append_data(2, 0.0);
        ppc->tvp_death_home->append_data(3, 0.0);
        ppc->tvp_death_home->append_data(4, 0.0);
        ppc->tvp_death_home->append_data(5, 0.0);
        ppc->tvp_death_home->append_data(6, 0.0);
        ppc->tvp_death_home->append_data(7, 0.05);
        ppc->tvp_death_home->append_data(8, 0.22);
    }
    // fill missing begin and/or end days in tvp_hosp_frac
    // advance if good_days() and good_integrity()
    // else, print error message
    if ( ppc->tvp_death_home->fill_missing_days() ){
        ppc->tvp_death_home->advance_indices(); // start day_index and data_index at 0
    } else {
        fprintf(stderr,"\n\n\tError in parsing contact rates.\n\tPlease check all -tv-death-home-prob-* command-line options or unset all these arguments to use the default values.\n\n");
        ppc->tvp_death_home->print_all();
        exit(-1);
    }
    for (int ac = 0; ac < NUMAC; ac++){
    ...
    // DEATH RATES
    // TODO: check life expectancy for the US
    ppc->v_deathrate[0] = 1.0/(365 * 85);
    ppc->v_deathrate[1] = 1.0/(365 * 75);
    ppc->v_deathrate[2] = 1.0/(365 * 65);
    ppc->v_deathrate[3] = 1.0/(365 * 55);
    ppc->v_deathrate[4] = 1.0/(365 * 45);
    ppc->v_deathrate[5] = 1.0/(365 * 35);
    ppc->v_deathrate[6] = 1.0/(365 * 25);
    ppc->v_deathrate[7] = 1.0/(365 * 15);
    ppc->v_deathrate[8] = 1.0/(365 * 5);
    ...
    if( G_B_DEATHRATE_OUTPUT )
    {
        ...
        // fprintf(stderr,"\n\n\tNeed to take care of new hosp-frac (using v_prob_I2_H_treatment)!!!\n\n");
        // exit(-1);
