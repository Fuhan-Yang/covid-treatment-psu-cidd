...
public:    
    ...
    bool vaccine_wastage_flag = true; // toggle vaccine wastage (i.e. vaccinating seropositive individuals)
    vector<double> v;			// this holds all the parameters; they are indexed by the enums above

    vector<double> v_pop_ac;

    // vector<double> v_treatment_coverage; // treatment coverage for each age group
    time_varying_prms_age<double> *tvp_treatment_coverage; // time-varying treatment coverage for each age group
    vector<double> v_treatment_success; // treatment efficacy for each age group

    vector<double> v_deathrate; // natural death rates for nine age groups
    ...
    time_varying_prms_age<double> *tvp_contact_rate;
    time_varying_prms_age<double> *tvp_contact_coeff;
    time_varying_prms_age<double> *tvp_hosp_frac;
    time_varying_prms_age<double> *tvp_vac_num;
    time_varying_prms_age<double> *tvp_death_home;
    time_varying_prms_nonage<double> *tvp_vac_efficacy;
    time_varying_prms_nonage<double> *tvp_dev_icu_frac;
    time_varying_prms_nonage<double> *tvp_dev_len_hospstay;
    time_varying_prms_nonage<double> *tvp_incubation_period;
    time_varying_prms_nonage<double> *tvp_infectious_period; // total infectious period, i.e. total time spent in I sub-classes

    vector<double> v_cumul_vac;
    ...
    vector<double> v_prob_I2_H_treatment;         // fraction of infected & symp individuals who are hospitalized after class I_2, taking into account at-home treatment
    ...
    void calculate_prob_I2_H_treatment();  // ensure 0 <= v_prob_I2_H_treatment <= 1,  0 <= v_treatment_success <= 1, 0 <= v_treatment_coverage <= 1
