...
## The introduction day of the first case: 2020-02-27 ## 
...
const.prms.start <- c("", 2,
                      ...
                      "0\t0",
                      "0\t0",
                      # "0.0130658513676206\t0.002874487",
                      # "0.0421950867804589\t0.01097072",
                      # "0.22715636067416\t0.1044919",
...
beta.strt <- c(1.720765,0.6847355,0.2626981,0.1098193,0.1397066,0.0728003,
               ...
               0.2112015,3.770088e-05,0.0566238,0.08361889,0.2265137,0.4272311,
               0.66382,0.6810689,0.8439437,0.8044479,1.374072,0.5063189,
               0.5757708,0.408598,0.4615122,0.3511908,0.2963026,0.5102841,
               0.433879,0.2818043,0.2763522,0.5218882,0.2538979,0.4913992,
               0.4851807,0.5700855,0.267635,0.1690315,1.53478,0.3349618,
               0.546698,0.351479,0.9901869,1.090582,0.9007221,1.249396,0.8405292,
               1.882964,0.5020716,2.003855,1.083612,0.7188044,1.331122,1.112111,
               0.7025387,1.092197,1.256292,0.6429194,1.221498,0.925158,0.9178373,
               0.8445321,0.7864919,0.744067,0.7083338,0.6973935,0.8407097,
               0.1912284,1.545183,0.5909206,0.4365773,1.150561,0.5423841,
               0.8545704,0.9185831,0.5238708,0.7960755,0.690585,0.6173245,
               0.9425083,0.4015244,1.276561,0.6832644,0.7189164,0.7285736,
               0.6809839,0.5979404,0.852439,0.9875087,0.4209666,1.219489,
               0.7758521,0.9662031)

odepath = 'cpp-v10-v-class/'
test_out = traj.from.params(Z %*% beta.strt,
                            params = prms.start,
...
### compare with RI data ###
ri.data = read.csv('data/RI/RI-20221208.csv')
ri.data$date = as.Date(ri.data$date)

updated_data = data.frame(date = ri.data$date,
                          new_cases = c(0,diff(ri.data$cumulative_confirmed_cases,1)),
                          new_hosp = c(0,diff(ri.data$hospitalized_cumulative,1)),
                          new_deaths = c(0,diff(ri.data$cumulative_deaths,1)))

updated_out = data.frame(date = primary_traj$date,
...
us_data = read.csv('data/US/aggregated_us_covid_data_20221214.csv')
us_data$date = as.Date(us_data$date)

primary_traj_us = get_ode_main_traj(test_out,scale_up = T)

updated_data_us = data.frame(date = us_data$date,
                          new_cases = c(rep(0,7),diff(us_data$cumu_cases,7)),
                          new_hosp = c(0,diff(us_data$cumu_hosp,1)),
                          new_deaths = c(rep(0,7),diff(us_data$cumu_deaths,7)))

updated_data_us = na.omit(updated_data_us)

updated_out_us = data.frame(date = primary_traj_us$date,
                         new_cases = c(rep(0,7),diff(primary_traj_us$cumu_cases,7)),
                         new_hosp = c(0,diff(primary_traj_us$cumu_hosp,1)),
                         new_deaths = c(rep(0,7),diff(primary_traj_us$cumu_death,7)))

#### plotting: supp fig #####
metrics = c('New Cases','New Hospitalized',"New Deaths")
mains = c('RI','','')
text_loc = c(2000,20,8)
jpeg(filename = 'figs/supp_fig_calibration.jpg',width = 12,height = 9.6,
     units = 'in',res = 300)
par(mfcol = c(3,2),mar = c(1.5,3,1.5,0),oma = c(1,0,1,0.5),
    cex.axis = 1.2)
for(i in 2:ncol(updated_out)) {
  
  plot(updated_data$date,updated_data[,i],type = 'l',
       main = '',xaxt = 'n',xlab = '',ylab = '',mgp = c(1.5,0.5,0))
  title(main = mains[i-1],line = 0.5,cex.main = 1.5)
  axis(side = 1,at = seq(as.Date('2020-03-01'),as.Date('2022-12-01'),'3 months'),
       labels = format(seq(as.Date('2020-03-01'),as.Date('2022-12-01'),'3 months'),'%Y-%m'))
  mtext(metrics[i-1],side = 2, line = 1.5,adj = 0, cex = 1, at = text_loc[i-1])
  points(updated_out$date,updated_out[,i],col = 'red',
         pch = 20,cex = 0.3)
  abline(v = as.Date('2021-06-06'),
         col = 'grey',lty = 'dashed')
  
}

for(i in 2:ncol(updated_out_us)) {
  
  if(i == 2) {
    plot(updated_data_us$date,updated_data_us[,i],type = 'l',
         main = 'US',xaxt = 'n',ylab = '',xlab = '',cex.main = 1.5,
         mgp = c(1.5,0.5,0))
  } else {
  plot(updated_data_us$date,updated_data_us[,i],type = 'l',
       main = '',xaxt = 'n',ylab ='',xlab = '',mgp = c(1.5,0.5,0))
  }
  axis(side = 1,at = seq(as.Date('2020-03-01'),as.Date('2022-12-01'),'3 months'),
       labels = format(seq(as.Date('2020-03-01'),as.Date('2022-12-01'),'3 months'),'%Y-%m'))
  points(updated_out_us$date,updated_out_us[,i],col = 'red',
         pch = 20,cex = 0.3)
  abline(v = as.Date('2021-06-06'),
         col = 'grey',lty = 'dashed')
